...
public partial class Admin_Servce_ServceMain : System.Web.UI.Page
{
    ...
    {
        SqlHelper her = new SqlHelper();
        string sql = @"select * from ML_Video a join ML_VideoClass b on a.sid0=b.nID
        ...
        if (DropName.SelectedItem.Value != "-1") { sql += " and a.oNewest=" + DropName.SelectedItem.Value + ""; }
        if (DropNew.SelectedItem.Value != "-1") { sql += " and a.oTop=" + DropNew.SelectedItem.Value + ""; }
    ...
    protected void gridField_DataBound(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < gridField.Rows.Count; i++)
        {
            ...
            Label lbtMemo = gridField.Rows[i].FindControl("lbtMemo") as Label;
            ...
            Label lblzhiding = gridField.Rows[i].FindControl("lblzhiding") as Label;
            if (lblzhiding.Text == "1")
            {
                lblzhiding.Text = "是";
            }
            else
            ...
            Label lblnew = gridField.Rows[i].FindControl("lblnew") as Label;
            if (lblnew.Text == "1")
            {
                lblnew.Text = "是";
            }
    ...
    protected void gridField_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            Response.Redirect("ServceEdit.aspx?id=" + e.CommandArgument.ToString() + "&ceid=" + gridField.PageIndex);
        }
    ...
    {
        Response.Redirect("ServceEdit.aspx?ceid=" + gridField.PageIndex);
    }
