...
// 一、<input name=txt id="txt"><input type=button value=setday onclick="setday(this,document.getElementById('txt')">
...
strFrame += '             <td style="cursor:hand" onclick="parent.tmpSelectHourInnerHTML(this)"';
...
strFrame += '             <td style="cursor:hand" onclick="parent.tmpSelectMinuteInnerHTML(this)"';
strFrame += ' onmouseover="style.backgroundColor=\'#aaccf3\'" onmouseout="style.backgroundColor=\'#63A3E9\'"';
strFrame += ' title="点击这里选择时间" align=center width=42>';
strFrame += '     <span id=meizzMinuteHead></span></td>';
strFrame += '             <td style="cursor:hand" onclick="parent.tmpSelectSecondInnerHTML(this)"';
strFrame += ' onmouseover="style.backgroundColor=\'#aaccf3\'" onmouseout="style.backgroundColor=\'#63A3E9\'"';
strFrame += ' title="点击这里选择时间" align=center width=42>';
strFrame += '     <span id=meizzSecondHead></span></td>';
strFrame += '    </tr></table></td></tr></table></div>';
var endDateLayer = document.getElementById('endDateLayer')
endDateLayer.contentWindow.document.write(strFrame);
endDateLayer.contentWindow.document.close(); //解决ie进度条不结束的问题
//==================================================== WEB 页面显示部分 ======================================================
var outObject;
var outButton; //点击的按钮
var outDate = ""; //存放对象的日期
var bUseTime = false; //是否使用时间
var odatelayer = endDateLayer.contentWindow.document
bImgSwitch();
...
function meizzWriteHead(yy, mm, ss) //往 head 中写入当前的年与月
{
    ...
    odatelayer.getElementById('meizzMonthHead').innerHTML = format(mm) + " 月";
    //插入当前小时、分
    odatelayer.getElementById('meizzHourHead').innerHTML = bUseTime ? (meizzTheHour + " 时") : "";
    odatelayer.getElementById('meizzMinuteHead').innerHTML = bUseTime ? (meizzTheMinute + " 分") : "";
    odatelayer.getElementById('meizzSecondHead').innerHTML = bUseTime ? (meizzTheSecond + " 秒") : "";
}
function tmpSelectYearInnerHTML(td) //年份的下拉框
{
    var strYear = parseInt(td.getElementsByTagName('span')[0].innerHTML);
    ...
    var s = "&nbsp;<select id=tmpSelectYear style='font-size: 12px' "
...
function tmpSelectMonthInnerHTML(td) //月份的下拉框
{
    var strMonth = parseInt(td.getElementsByTagName('span')[0].innerHTML);
    var m = (strMonth) ? strMonth : new Date().getMonth() + 1;
    var s = "&nbsp;&nbsp;&nbsp;<select id=tmpSelectMonth style='font-size: 12px' "
...
/***** 增加 小时、分钟 ***/
function tmpSelectHourInnerHTML(td) //小时的下拉框
{
    if (!bUseTime) { return; }
    var strHour = parseInt(td.getElementsByTagName('span')[0].innerHTML);
    //if (strHour.match(/\D/) != null) { alert("小时输入参数不是数字！"); return; }
    var m = (strHour) ? strHour : new Date().getHours();
    var s = "<select id=tmpSelectHour style='font-size: 12px' "
    ...
    odatelayer.getElementById('tmpSelectHourLayer').innerHTML = selectInnerHTML;
    odatelayer.getElementById('tmpSelectHour').focus();
}
function tmpSelectMinuteInnerHTML(td) //分钟的下拉框
{
    if (!bUseTime) { return; }
    var strMinute = parseInt(td.getElementsByTagName('span')[0].innerHTML);
    var m = (strMinute) ? strMinute : new Date().getMinutes();
    var s = "<select id=tmpSelectMinute style='font-size: 12px' "
    ...
    selectInnerHTML += "</select>";
    odatelayer.getElementById('tmpSelectMinuteLayer').style.display = "";
    odatelayer.getElementById('tmpSelectMinuteLayer').innerHTML = selectInnerHTML;
    odatelayer.getElementById('tmpSelectMinute').focus();
}
function tmpSelectSecondInnerHTML(td) //秒的下拉框
{
    if (!bUseTime) { return; }
    var strSecond = parseInt(td.getElementsByTagName('span')[0].innerHTML);
    var m = (strSecond) ? strSecond : new Date().getMinutes();
    var s = "<select id=tmpSelectSecond style='font-size: 12px' "
    s += "onblur='document.getElementById(\"tmpSelectSecondLayer\").style.display=\"none\"' "
    s += "onchange='document.getElementById(\"tmpSelectSecondLayer\").style.display=\"none\";"
    ...
    selectInnerHTML += "</select>";
    odatelayer.getElementById('tmpSelectSecondLayer').style.display = "";
    odatelayer.getElementById('tmpSelectSecondLayer').innerHTML = selectInnerHTML;
    odatelayer.getElementById('tmpSelectSecond').focus();
}
...
{
    odatelayer.getElementById('meizzHourHead').innerHTML = meizzTheHour + " 时";
    odatelayer.getElementById('meizzMinuteHead').innerHTML = meizzTheMinute + " 分";
    odatelayer.getElementById('meizzSecondHead').innerHTML = meizzTheSecond + " 秒";
...
{
    odatelayer.getElementById('meizzHourHead').innerHTML = '';
    odatelayer.getElementById('meizzMinuteHead').innerHTML = '';
    odatelayer.getElementById('meizzSecondHead').innerHTML = '';
    WriteDateTo(meizzTheYear, meizzTheMonth, meizzTheDate, meizzTheHour, meizzTheMinute, meizzTheSecond)
}
function evaSetTimeNow() //设置时间控件为当前时间
{
    odatelayer.getElementById('meizzHourHead').innerHTML = new Date().getHours() + " 时";
    odatelayer.getElementById('MinuteHead').innerHTML = new Date().getMinutes() + " 分";
    odatelayer.getElementById('meizzSecondHead').innerHTML = new Date().getSeconds() + " 秒";
